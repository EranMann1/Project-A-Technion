function [Currents] = currentsOfPlaneWave(waveguide,k,k_y,N,eta,r_eff,Lambda) 
    ...
    Vec_diag = Zs-1j*k*eta/(2*pi)*log(2*pi*r_eff/Lambda)+k*eta/2*(1./(Lambda*conj(sqrt(k^2-Ky.^2)))+Sum4D);
    for index = 1:12
    ...
    MatrixA_nondiag = k*eta/(2*Lambda)*sum(exp(-1j*Alphas.*MatrixDs).*exp(-1j*Betas.*MatrixHs)./Betas,4);
    ...
    PW_vec = exp(-1j*(conj(sqrt(k^2-Ky.^2)).*transpose(Hs)+Ky.*transpose(Ds)));
